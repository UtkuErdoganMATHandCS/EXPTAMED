...
class EXPTAMED_v3(torch.optim.Optimizer):
    ...
    def __init__(self, params, lr=1e-1, eta=0, beta=1e14, r=3, eps=1e-8, weight_decay=0, sync_eps=False, averaging=False):
        ...
        super(EXPTAMED_v3, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(EXPTAMED_v3, self).__setstate__(state)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if eta > 0:
                    reg = torch.mul(p.data,eta *total_norm/( 1+ math.sqrt(lr)*total_norm*torch.abs(p.data)) ) #    regularization term here  , taken lambda or sqrt lambda,
                ...
                numer = torch.sign(grad) * ( 1 + lr/ (eps+ abs_grad)) # boosting here, taken lambda or sqrt lambda,
                #numer=G*(1+sqrt(lambda)/(eps+|G|) )
                factor= (exp_abs_minus_grad_lr-1)  
