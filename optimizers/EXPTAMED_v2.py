...
class EXPTAMED_v2(torch.optim.Optimizer):
    ...
    def __init__(self, params, lr=1e-1, eta=0, beta=1e14, r=3, eps=1e-8, weight_decay=0, sync_eps=False, averaging=False):
        ...
        super(EXPTAMED_v2, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(EXPTAMED_v2, self).__setstate__(state)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                drift=grad
                if eta > 0:
                    drift=torch.add(grad,p.data,alpha=eta*total_norm)
                ...
                exp_abs_minus_drift_sqrtlr=torch.exp(-math.sqrt(lr)*abs_drift)
                abs_grad=torch.abs(grad)
               

                noise = math.sqrt(2 * lr / beta) * torch.randn(size=p.size(), device=device)*exp_abs_minus_drift_sqrtlr
                ...
                numer = torch.sign(drift) * ( 1 + math.sqrt(lr)/ (eps+ abs_grad)) # boosting here, taken lambda or sqrt lambda,
                #numer=G*(1+sqrt(lambda)/(eps+|G|) )
                factor= (exp_abs_minus_drift_sqrtlr-1)  
